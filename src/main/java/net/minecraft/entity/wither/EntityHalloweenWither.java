...
public class EntityHalloweenWither
  extends EntityMob
  implements IRangedAttackMob
{
  ...
  private static final Predicate attackEntitySelector = new Predicate()
  {
    ...
    {
      return (((p_180027_1_ instanceof EntityLivingBase)) && (((EntityLivingBase)p_180027_1_).getCreatureAttribute() != EnumCreatureAttribute.UNDEAD) && !(p_180027_1_ instanceof EntityBat) && !(p_180027_1_ instanceof EntityWitch));
    }
  ...
  public EntityHalloweenWither(World worldIn)
  {
    super(worldIn);
  ...
  public void onLivingUpdate()
  {
    ...
    this.worldObj.setWorldTime(13000L);
    for (int i = 0; i < 2; i++)
    {
    ...
    for (int j = 0; j < 15; j++)
    {
      ...
      if ((flag) && (this.worldObj.rand.nextInt(4) == 0)) {
        this.worldObj.spawnParticle(EnumParticleTypes.SMOKE_LARGE, d10 + this.rand.nextGaussian() * 0.30000001192092896D, d2 + this.rand.nextGaussian() * 0.30000001192092896D, d4 + this.rand.nextGaussian() * 0.30000001192092896D, 0.699999988079071D, 0.699999988079071D, 0.5D, new int[0]);
      }
    }
    if (getInvulTime() > 0) {
      for (int j = 0; j < 15; j++) {
  ...
  protected void updateAITasks()
  {
    ...
    {
      super.updateAITasks();
      if (!this.worldObj.isRemote && this.rand.nextInt(4) == 0 && this.ticksExisted % 20 == 0)
      {
      	if (this.worldObj.getCurrentDate().get(2) + 1 == 10 && this.worldObj.getCurrentDate().get(5) >= 20 || this.worldObj.getCurrentDate().get(2) + 1 == 11 && this.worldObj.getCurrentDate().get(5) <= 3)
      	{
      	    for (int i = 0; i < 8; i++)
      	    {
      	    	EntityBat blazeminion = new EntityBat(this.worldObj);
      ...
      if (!this.worldObj.isRemote && this.rand.nextInt(15) == 0 && this.ticksExisted % 20 == 0)
      {
      	if (this.worldObj.getCurrentDate().get(2) + 1 == 10 && this.worldObj.getCurrentDate().get(5) >= 20 || this.worldObj.getCurrentDate().get(2) + 1 == 11 && this.worldObj.getCurrentDate().get(5) <= 3)
      	{
      	    for (int i = 0; i < 2; i++)
      	    {
      	    	EntityWitch blazeminion = new EntityWitch(this.worldObj);
      	    	blazeminion.func_180482_a(this.worldObj.getDifficultyForLocation(new BlockPos(this)), (IEntityLivingData)null);
      	    	blazeminion.copyLocationAndAnglesFrom(this);
      	    	blazeminion.posY = this.posY + 2D;
      	        double d1 = blazeminion.posX - this.posX;
  ...
  private void launchWitherSkullToCoords(int p_82209_1_, double p_82209_2_, double p_82209_4_, double p_82209_6_, boolean p_82209_8_)
  {
    ...
    EntitySpookySkull entitywitherskull = new EntitySpookySkull(this.worldObj, this, d6, d7, d8);
    if (p_82209_8_) {
  ...
  public boolean attackEntityFrom(DamageSource source, float amount)
  {
    ...
    if ((source != DamageSource.fall) && (source != DamageSource.anvil) && (source != DamageSource.fallingBlock) && (source != DamageSource.drown) && (source != DamageSource.inWall) && (!(source.getEntity() instanceof EntityHalloweenWither)))
    {
  ...
  {
    dropItem(Items.pumpkin_pie, 128);
