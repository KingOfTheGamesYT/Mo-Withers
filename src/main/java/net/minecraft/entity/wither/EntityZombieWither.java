...
public class EntityZombieWither
  extends EntityMob
  implements IBossDisplayData, IRangedAttackMob
{
  ...
  private static final Predicate attackEntitySelector = new Predicate()
  {
    ...
    {
      return ((p_180027_1_ instanceof EntityLivingBase && !(p_180027_1_ instanceof EntityZombie) && !(p_180027_1_ instanceof EntityZombieWither)));
    }
  ...
  public EntityZombieWither(World worldIn)
  {
    super(worldIn);
    ...
    this.targetTasks.addTask(0, new EntityAINearestAttackableTarget(this, EntityIronGolem.class, false));
  ...
  {
	  this.playSound("mob.wither.idle", 0.5F, this.getSoundPitch() + 0.1F);
    return "mob.zombie.say";
  }
  
  protected String getHurtSound()
  {
	  this.playSound("mob.wither.hurt", 0.5F, this.getSoundPitch() + 0.1F);
    return "mob.zombie.hurt";
  }
  
  protected String getDeathSound()
  {
	  this.playSound("mob.wither.death", 0.5F, this.getSoundPitch() + 0.1F);
  ...
  public void onLivingUpdate()
  {
      ...
      if (entity != null)
      {
        ...
        if ((list111 != null) && (!list111.isEmpty())) {
          for (int i111 = 0; i111 < list111.size(); i111++)
          {
            ...
            if (entity11 != null && entity11 instanceof EntityZombie && entity instanceof EntityLivingBase)
            {
    ...
    if (this.motionX * this.motionX + this.motionZ * this.motionZ != 0.0D) {
      ...
      if (!this.worldObj.isRemote && this.rand.nextInt(5) == 0 && this.ticksExisted % 20 == 0)
      {
    	  EntityZombie blazeminion = new EntityZombie(this.worldObj);
    ...
    else
    {
      ...
      for (int i = 1; i < 3; i++) {
        if (this.ticksExisted >= this.field_82223_h[(i - 1)])
        {
          ...
          if ((this.worldObj.getDifficulty() == EnumDifficulty.NORMAL) || (this.worldObj.getDifficulty() == EnumDifficulty.HARD) || this.villageObj != null)
          {
            int k2 = i - 1;
            ...
            if (l2 > 15)
            {
              ...
              if (this.villageObj != null)
              {
            	  ...
            	  f1 = 2.0F;
            	  BlockPos blockpos = this.villageObj.getCenter();
                  d0 = MathHelper.getRandomDoubleInRange(this.rand, blockpos.getX() - f, blockpos.getX() + f);
                  d1 = MathHelper.getRandomDoubleInRange(this.rand, blockpos.getY() - f1, blockpos.getY() + f1);
                  d2 = MathHelper.getRandomDoubleInRange(this.rand, blockpos.getZ() - f, blockpos.getZ() + f);
            	  launchWitherSkullToCoords(i + 1, d0, d1, d2, true);
            	  this.field_82223_h[(i - 1)] = (this.ticksExisted + 3);
              }
              else
              {
            	  launchWitherSkullToCoords(i + 1, d0, d1, d2, true);
      ...
      if (this.ticksExisted % 10 == 0)
      {
        ...
        if (this.villageObj != null)
        {
          ...
          float f1 = 2.0F;
    	  BlockPos blockpos = this.villageObj.getCenter();
          double d0 = MathHelper.getRandomDoubleInRange(this.rand, blockpos.getX() - f, blockpos.getX() + f);
          double d1 = MathHelper.getRandomDoubleInRange(this.rand, blockpos.getY() - f1, blockpos.getY() + f1);
          double d2 = MathHelper.getRandomDoubleInRange(this.rand, blockpos.getZ() - f, blockpos.getZ() + f);
          launchWitherSkullToCoords(0 + this.rand.nextInt(2), d0, d1, d2, true);
        }
      }
    }
  }
  ...
  {
	  this.playSound("mob.zombie.hurt", 1.0F, 1.0F);
    ...
    EntityWitherSkullZombie entitywitherskull = new EntityWitherSkullZombie(this.worldObj, this, d6, d7, d8);
    if (p_82209_8_) {
  ...
  public boolean attackEntityFrom(DamageSource source, float amount)
  {
    ...
    if ((source != DamageSource.starve) && (!(source.getEntity() instanceof EntityZombie)) && (!(source.getEntity() instanceof EntityZombieWither)))
    {
      ...
      if ((entity != null) && (!(entity instanceof EntityPlayer)) && ((entity instanceof EntityLivingBase)) && ((entity instanceof EntityCreeper) || (entity instanceof EntityZombieWither))) {
        return false;
      }
      if (this.blockBreakCounter <= 0) {
