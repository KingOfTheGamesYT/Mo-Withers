...
public class EntityWaterWither
  extends EntityMob
  implements IBossDisplayData, IRangedAttackMob
{
  ...
  private static final Predicate attackEntitySelector = new Predicate()
  {
    ...
    {
      return ((p_180027_1_ instanceof EntityLivingBase)) && (((EntityLivingBase)p_180027_1_).getCreatureAttribute() != EnumCreatureAttribute.UNDEAD) && (!(p_180027_1_ instanceof EntityWaterMob) && (!(p_180027_1_ instanceof EntityGuardian)));
    }
  ...
  public EntityWaterWither(World worldIn)
  {
    super(worldIn);
  ...
  public void onLivingUpdate()
  {
    ...
    if ((!this.worldObj.isRemote) && (getWatchedTargetId(0) > 0))
    {
      ...
      {
        if ((this.posY < entity.posY) || (!isArmored() && !canConjoin() && (this.posY < entity.posY + 7.0D)))
        {
    ...
    for (int j = 0; j < 3; j++)
    {
      ...
      {
          this.worldObj.spawnParticle(EnumParticleTypes.WATER_SPLASH, d10 + this.rand.nextGaussian() * 0.30000001192092896D, d2 + this.rand.nextGaussian() * 0.30000001192092896D, d4 + this.rand.nextGaussian() * 0.3D, 0.0D, 0.0D, 0.0D, new int[0]);
          ...
          if ((flag) || (this.worldObj.rand.nextInt(20) == 0))
          {
            this.worldObj.spawnParticle(EnumParticleTypes.DRIP_WATER, d10 + this.rand.nextGaussian() * 0.30000001192092896D, d2 + this.rand.nextGaussian() * 0.30000001192092896D, d4 + this.rand.nextGaussian() * 0.6D, 0.0D, 0.0D, 0.0D, new int[0]);
            this.worldObj.spawnParticle(EnumParticleTypes.DRIP_WATER, this.posX + (this.rand.nextDouble() - 0.5D) * this.width, this.posY + this.rand.nextDouble() * this.height, this.posZ + (this.rand.nextDouble() - 0.5D) * this.width, 0.0D, 0.0D, 0.0D, new int[0]);
          }
      }
    }
    if (getInvulTime() > 0) {
      for (int j = 0; j < 15; j++) {
          ...
          {
              this.worldObj.spawnParticle(EnumParticleTypes.DRIP_WATER, this.posX + this.rand.nextGaussian() * 1.0D, this.posY + this.rand.nextFloat() * 3.3F, this.posZ + this.rand.nextGaussian() * 0.6D, 0.0D, 0.0D, 0.0D, new int[0]);
          }
      }
    }
    if (canConjoin()) {
  ...
  protected void updateAITasks()
  {
    ...
    else
    {
      ...
      if (this.isBurning() || this.isInLava())
      {
        ...
        attackEntityFrom(DamageSource.onFire, 10.0F);
      }
      for (int i = 1; i < 3; i++) {
      ...
      if (this.worldObj.getBiomeGenForCoords(new BlockPos(i11, 0, k)).getFloatTemperature(new BlockPos(i11, j, k)) > 1.0F) {
        attackEntityFrom(DamageSource.onFire, 5.0F);
      }
  ...
  {
    return (block != Blocks.water) && (block != Blocks.flowing_water) && (block != Blocks.barrier) && (block != Blocks.bedrock) && (block != Blocks.end_portal) && (block != Blocks.end_portal_frame) && (block != Blocks.command_block);
  }
  ...
  {
    this.playSound(this.getSplashSound(), 5F, 1.25F);
    ...
    EntityWaterSkull entitywitherskull = new EntityWaterSkull(this.worldObj, this, d6, d7, d8);
    if (p_82209_8_) {
  ...
  public boolean attackEntityFrom(DamageSource source, float amount)
  {
	  if (amount >= 1F && (source == DamageSource.inFire || source == DamageSource.onFire ||  source == DamageSource.lava ||  source == DamageSource.lightningBolt))
	    {
		  amount *= 3F;
	        this.playSound("random.fizz", 0.25F, 2.0F);
	    }
    if (isEntityInvulnerable(source)) {
      return false;
    }
    if ((source != DamageSource.drown) && (!(source.getEntity() instanceof EntityWaterWither)))
    {
  ...
  protected void dropFewItems(boolean p_70628_1_, int p_70628_2_)
  {
      ...
      {
          this.entityDropItem(new ItemStack(Blocks.sponge, 1, 1), 3F);
      }
    ...
    if (!this.worldObj.isRemote)
    {
        ...
        while (iterator.hasNext())
        {
            ...
            entityplayer.triggerAchievement(MoWithersAchievments.achievementKillWaterWither);
        }
    }
  }
