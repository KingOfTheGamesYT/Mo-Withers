  ...
  implements IRangedAttackMob
{
  ...
  public EntityCoalWither(World worldIn)
  {
    super(worldIn);
  ...
  public void onLivingUpdate()
  {
    ...
    for (int j = 0; j < 3; j++)
    {
      ...
      {
          this.worldObj.spawnParticle(EnumParticleTypes.LAVA, d10 + this.rand.nextGaussian() * 0.30000001192092896D, d2 + this.rand.nextGaussian() * 0.30000001192092896D, d4 + this.rand.nextGaussian() * 0.3D, 0.0D, 0.0D, 0.0D, new int[0]);
      ...
      if ((flag) && (this.worldObj.rand.nextInt(4) == 0)) {
        this.worldObj.spawnParticle(EnumParticleTypes.BLOCK_CRACK, d10 + this.rand.nextGaussian() * 0.30000001192092896D, d2 + this.rand.nextGaussian() * 0.30000001192092896D, d4 + this.rand.nextGaussian() * 0.30000001192092896D, 0.699999988079071D, 0.699999988079071D, 0.5D, new int[] {173});
      }
    }
    if (getInvulTime() > 0) {
      for (int j = 0; j < 15; j++) {
        this.worldObj.spawnParticle(EnumParticleTypes.FLAME, this.posX + this.rand.nextGaussian() * 1.0D, this.posY + this.rand.nextFloat() * 3.3F, this.posZ + this.rand.nextGaussian() * 0.6D, 0.0D, 0.0D, 0.0D, new int[] {42});
      }
    }
  }
  ...
  protected void updateAITasks()
  {
    ...
    else
    {
      ...
      if (this.isBurning() || this.isInLava())
      {
        attackEntityFrom(DamageSource.onFire, 30.0F);
        ...
        for (int l1 = -3; l1 <= 3; l1++) {
          for (int i2 = -4; i2 <= 8; i2++) {
            for (int j = -3; j <= 3; j++)
            {
              ...
              Block block1 = this.worldObj.getBlockState(pos.offset(EnumFacing.UP)).getBlock();
              Block block2 = this.worldObj.getBlockState(pos.offset(EnumFacing.DOWN)).getBlock();
              Block block3 = this.worldObj.getBlockState(pos.offset(EnumFacing.EAST)).getBlock();
              Block block4 = this.worldObj.getBlockState(pos.offset(EnumFacing.NORTH)).getBlock();
              Block block5 = this.worldObj.getBlockState(pos.offset(EnumFacing.SOUTH)).getBlock();
              Block block6 = this.worldObj.getBlockState(pos.offset(EnumFacing.WEST)).getBlock();
              if (block.isFlammable(worldObj, pos, EnumFacing.UP) && block1 == Blocks.air) {
              	this.worldObj.setBlockState(pos.offset(EnumFacing.UP), Blocks.fire.getDefaultState(), 3);
              }
              if (block.isFlammable(worldObj, pos, EnumFacing.DOWN) && block2 == Blocks.air) {
                	this.worldObj.setBlockState(pos.offset(EnumFacing.DOWN), Blocks.fire.getDefaultState(), 3);
                }
              if (block.isFlammable(worldObj, pos, EnumFacing.EAST) && block3 == Blocks.air) {
                	this.worldObj.setBlockState(pos.offset(EnumFacing.EAST), Blocks.fire.getDefaultState(), 3);
                }
              if (block.isFlammable(worldObj, pos, EnumFacing.NORTH) && block4 == Blocks.air) {
                	this.worldObj.setBlockState(pos.offset(EnumFacing.NORTH), Blocks.fire.getDefaultState(), 3);
                }
              if (block.isFlammable(worldObj, pos, EnumFacing.SOUTH) && block5 == Blocks.air) {
                	this.worldObj.setBlockState(pos.offset(EnumFacing.SOUTH), Blocks.fire.getDefaultState(), 3);
                }
              if (block.isFlammable(worldObj, pos, EnumFacing.WEST) && block6 == Blocks.air) {
                	this.worldObj.setBlockState(pos.offset(EnumFacing.WEST), Blocks.fire.getDefaultState(), 3);
                }
            }
          }
        }
  ...
  {
    return (block != Blocks.coal_block) && (block != Blocks.coal_ore) && (block != Blocks.barrier) && (block != Blocks.bedrock) && (block != Blocks.end_portal) && (block != Blocks.end_portal_frame) && (block != Blocks.command_block);
  }
  ...
  private void launchWitherSkullToCoords(int p_82209_1_, double p_82209_2_, double p_82209_4_, double p_82209_6_, boolean p_82209_8_)
  {
    ...
    EntityMettalicSkull entitywitherskull = new EntityMettalicSkull(this.worldObj, this, d6, d7, d8);
    entitywitherskull.setSkullType(4);
  ...
  public boolean attackEntityFrom(DamageSource source, float amount)
  {
    ...
    if ((source != DamageSource.fall) && (source != DamageSource.anvil) && (source != DamageSource.fallingBlock) && (source != DamageSource.drown) && (source != DamageSource.inWall) && (!(source.getEntity() instanceof EntityCoalWither)))
    {
      ...
      playSound("dig.stone", 3.0F, getSoundPitch() - 0.6F);
  ...
  public void onDeath(DamageSource cause)
  {
      ...
      {
		  this.motionY = 1F;
		  this.worldObj.newExplosion(this, this.posX, this.posY + getEyeHeight(), this.posZ, 7.0F, true, this.worldObj.getGameRules().getGameRuleBooleanValue("mobGriefing"));
      }
  }
}
