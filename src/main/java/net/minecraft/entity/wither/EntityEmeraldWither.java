...
public class EntityEmeraldWither
  extends EntityMob
  implements IRangedAttackMob
{
  ...
  public EntityEmeraldWither(World worldIn)
  {
    super(worldIn);
    ...
    this.tasks.addTask(3, new EntityAIFleeSun(this, 1.5D));
  ...
  public void onLivingUpdate()
  {
    ...
    if ((!this.worldObj.isRemote) && (getWatchedTargetId(0) > 0))
    {
      ...
      if (entity != null)
      {
        if ((this.posY < entity.posY) || ((!isArmored()) && (this.posY < entity.posY + 4.0D)))
        {
    ...
    for (int j = 0; j < 3; j++)
    {
      ...
      if ((flag) && (this.worldObj.rand.nextInt(4) == 0)) {
        this.worldObj.spawnParticle(EnumParticleTypes.BLOCK_CRACK, d10 + this.rand.nextGaussian() * 0.30000001192092896D, d2 + this.rand.nextGaussian() * 0.30000001192092896D, d4 + this.rand.nextGaussian() * 0.30000001192092896D, 0.699999988079071D, 0.699999988079071D, 0.5D, new int[] {133});
      }
    }
    if (getInvulTime() > 0) {
      for (int j = 0; j < 15; j++) {
        this.worldObj.spawnParticle(EnumParticleTypes.LAVA, this.posX + this.rand.nextGaussian() * 1.0D, this.posY + this.rand.nextFloat() * 3.3F, this.posZ + this.rand.nextGaussian() * 0.6D, 0.0D, 0.0D, 0.0D, new int[] {42});
      }
    }
  }
  ...
  {
    return (block != Blocks.emerald_block) && (block != Blocks.emerald_ore) && (block != Blocks.barrier) && (block != Blocks.bedrock) && (block != Blocks.end_portal) && (block != Blocks.end_portal_frame) && (block != Blocks.command_block);
  }
  ...
  {
	  playSound("random.anvil_land", 3.0F, getSoundPitch() - this.rand.nextFloat() * 0.9F);
  ...
  public boolean attackEntityFrom(DamageSource source, float amount)
  {
    ...
    if ((source != DamageSource.fall) && (source != DamageSource.anvil) && (source != DamageSource.fallingBlock) && (source != DamageSource.drown) && (source != DamageSource.inWall) && (!(source.getEntity() instanceof EntityEmeraldWither)))
    {
  ...
  protected void applyEntityAttributes()
  {
    ...
    getEntityAttribute(SharedMonsterAttributes.maxHealth).setBaseValue(2400.0D);
