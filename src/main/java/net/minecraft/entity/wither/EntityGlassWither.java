...
public class EntityGlassWither
  extends EntityMob
  implements IRangedAttackMob
{
  ...
  public EntityGlassWither(World worldIn)
  {
    super(worldIn);
  ...
  public void onLivingUpdate()
  {
    if ((!this.worldObj.isRemote) && (getWatchedTargetId(0) > 0))
    {
      ...
      if (entity != null)
      {
    	  this.getLookHelper().setLookPositionWithEntity(entity, 10F, this.getVerticalFaceSpeed());
        ...
        if (d3 > 1.0D)
        {
          ...
          this.motionX += (d0 / d5 * 0.75D - this.motionX) * 0.75D;
          this.motionZ += (d1 / d5 * 0.5D - this.motionZ) * 0.75D;
        }
      }
    }
    ...
    for (int j = 0; j < 3; j++)
    {
      ...
      this.worldObj.spawnParticle(EnumParticleTypes.BLOCK_CRACK, d10 + this.rand.nextGaussian() * 0.30000001192092896D, d2 + this.rand.nextGaussian() * 0.30000001192092896D, d4 + this.rand.nextGaussian() * 0.3D, 0.0D, 0.0D, 0.0D, new int[] { 20 });
      
      this.worldObj.spawnParticle(EnumParticleTypes.BLOCK_CRACK, this.posX + (this.rand.nextDouble() - 0.5D) * this.width, this.posY + this.rand.nextDouble() * this.height, this.posZ + (this.rand.nextDouble() - 0.5D) * this.width, 0.0D, 0.0D, 0.0D, new int[] { 20 });
    }
    if (getInvulTime() > 0) {
      for (int j = 0; j < 15; j++) {
  ...
  {
    return (block != Blocks.sand) && (block != Blocks.sandstone) && (block != Blocks.sandstone_stairs) && (block != Blocks.cactus) && (block != Blocks.barrier) && (block != Blocks.bedrock) && (block != Blocks.end_portal) && (block != Blocks.end_portal_frame) && (block != Blocks.command_block);
  }
  ...
  {
	  playSound("dig.glass", 3.0F, getSoundPitch() - 0.5F);
  ...
  public boolean attackEntityFrom(DamageSource source, float amount)
  {
    ...
    if ((source != DamageSource.fall) && (source != DamageSource.anvil) && (source != DamageSource.fallingBlock) && (source != DamageSource.drown) && (source != DamageSource.inWall) && (!(source.getEntity() instanceof EntityGlassWither)))
    {
      ...
      for (int j1 = 0; j1 < 3; j1++)
      {
        ...
        double d2 = func_82208_v(j);
        double d4 = func_82213_w(j);
        this.worldObj.spawnParticle(EnumParticleTypes.BLOCK_CRACK, d10 + this.rand.nextGaussian() * 0.5D, d2 + this.rand.nextGaussian() * 0.5D, d4 + this.rand.nextGaussian() * 0.5D, 0.0D, 0.0D, 0.0D, new int[] { 20 });
        this.worldObj.spawnParticle(EnumParticleTypes.BLOCK_CRACK, this.posX + (this.rand.nextDouble() - 0.5D) * this.width, this.posY + this.rand.nextDouble() * this.height, this.posZ + (this.rand.nextDouble() - 0.5D) * this.width, 0.0D, 0.0D, 0.0D, new int[] { 20 });
      }
