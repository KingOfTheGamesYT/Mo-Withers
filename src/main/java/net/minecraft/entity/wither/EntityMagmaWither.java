...
public class EntityMagmaWither
  extends EntityMob
  implements IRangedAttackMob
{
  ...
  public EntityMagmaWither(World worldIn)
  {
    super(worldIn);
  ...
  public void moveEntityWithHeading(float p_70612_1_, float p_70612_2_)
  {
      if (this.isServerWorld())
      {
          if (this.isInLava())
          {
              this.moveFlying(p_70612_1_, p_70612_2_, 0.25F);
  ...
  {
      return this.worldObj.isMaterialInBB(this.getEntityBoundingBox().expand(-0.25D, -0.25D, -0.25D).offset(0D, 1.0D, 0D), Material.lava);
  }
  ...
  public void onLivingUpdate()
  {
	  if (this.isInLava())
		  this.heal(5F);
    this.motionY *= 0.35D;
    if ((this.rand.nextInt(100) == 0) && (!isSilent())) {
      this.worldObj.playSound(this.posX + 0.5D, this.posY + 2.0D, this.posZ + 0.5D, "liquid.lava", 0.2F + this.rand.nextFloat() * 0.2F, 0.9F + this.rand.nextFloat() * 0.15F, false);
    }
    if ((!this.worldObj.isRemote) && (getWatchedTargetId(0) > 0))
    {
      ...
      if (entity != null)
      {
        if ((this.posY < entity.posY) || ((!isArmored()) && (this.posY < entity.posY + 3.0D)))
        {
          if (this.motionY < 0.0D) {
            this.motionY = 0.0D;
          }
          this.motionY += (0.35D - this.motionY) * 0.35D;
        }
        ...
        if (d3 > 1.0D)
        {
          ...
          this.motionX += (d0 / d5 * 0.35D - this.motionX) * 0.35D;
          this.motionZ += (d1 / d5 * 0.35D - this.motionZ) * 0.35D;
        }
      }
    }
    ...
    for (int j = 0; j < 3; j++)
    {
      ...
      this.worldObj.spawnParticle(EnumParticleTypes.LAVA, this.posX + (this.rand.nextDouble() - 0.5D) * this.width, this.posY + this.rand.nextDouble() * this.height, this.posZ + (this.rand.nextDouble() - 0.5D) * this.width, 0.0D, 0.0D, 0.0D, new int[0]);
      this.worldObj.spawnParticle(EnumParticleTypes.DRIP_LAVA, d10 + this.rand.nextGaussian() * 0.30000001192092896D, d2 + this.rand.nextGaussian() * 0.30000001192092896D, d4 + this.rand.nextGaussian() * 0.3D, 0.0D, 0.0D, 0.0D, new int[0]);
  ...
  protected void updateAITasks()
  {
    ...
    else
    {
      ...
      if (isWet())
      {
        ...
        attackEntityFrom(DamageSource.drown, 50.0F);
      }
  ...
  {
    return (block != Blocks.lava) && (block != Blocks.flowing_lava) && (block != Blocks.fire) && (block != Blocks.barrier) && (block != Blocks.bedrock) && (block != Blocks.end_portal) && (block != Blocks.end_portal_frame) && (block != Blocks.command_block);
  }
  ...
  private void launchWitherSkullToCoords(int p_82209_1_, double p_82209_2_, double p_82209_4_, double p_82209_6_, boolean p_82209_8_)
  {
    ...
    EntityMagmaSkull entitywitherskull = new EntityMagmaSkull(this.worldObj, this, d6, d7, d8);
    if (p_82209_8_) {
  ...
  public boolean attackEntityFrom(DamageSource source, float amount)
  {
    ...
    if ((source != DamageSource.lava) && (source != DamageSource.inFire) && (source != DamageSource.onFire) && (!(source.getEntity() instanceof EntityMagmaWither)))
    {
  ...
  protected void dropFewItems(boolean p_70628_1_, int p_70628_2_)
  {
    ...
    if (!this.worldObj.isRemote)
    {
        ...
        while (iterator.hasNext())
        {
            ...
            entityplayer.triggerAchievement(MoWithersAchievments.achievementKillMagmaWither);
        }
    }
  }
  ...
  protected void onDeathUpdate()
  {
    if (isInWater())
    {
      ...
      for (int j = 0; j < 3; j++)
      {
        ...
        this.worldObj.setBlockState(new BlockPos(this.posX, this.posY, this.posZ), Blocks.obsidian.getDefaultState());
        this.worldObj.setBlockState(new BlockPos(this.posX, this.posY + 1.0D, this.posZ), Blocks.obsidian.getDefaultState());
        this.worldObj.setBlockState(new BlockPos(this.posX, this.posY + 2.0D, this.posZ), Blocks.obsidian.getDefaultState());
        this.worldObj.setBlockState(new BlockPos(this.posX, this.posY + 3.0D, this.posZ), Blocks.obsidian.getDefaultState());
        this.worldObj.setBlockState(new BlockPos(d10, d2, d4), Blocks.obsidian.getDefaultState());
      }
      if (!this.worldObj.isRemote)
      {
          Iterator iterator = this.worldObj.getEntitiesWithinAABB(EntityPlayer.class, this.getEntityBoundingBox().expand(50.0D, 100.0D, 50.0D)).iterator();
          ...
          while (iterator.hasNext())
          {
              EntityPlayer entityplayer = (EntityPlayer)iterator.next();
              entityplayer.triggerAchievement(AchievementList.killWither);
              entityplayer.triggerAchievement(MoWithersAchievments.achievementKillMagmaWitherWaterDeath);
          }
      }
      setDead();
    }
