...
public class EntityBedrockWither
  extends EntityMob
  implements IRangedAttackMob
{
  ...
  public EntityBedrockWither(World worldIn)
  {
    super(worldIn);
    ...
    this.tasks.addTask(2, new EntityAIAttackRanged(this, 1.0D, 120, 30.0F));
  ...
  {
    if ((!this.worldObj.isRemote) && (getWatchedTargetId(0) > 0) && this.isEntityAlive())
    {
      ...
      if (entity != null)
      {
        ...
        if (d3 > 64.0D && (entity.posY <= this.posY + 5.0D))
        {
          double d5 = MathHelper.sqrt_double(d3);
          this.motionX += (d0 / d5 * 0.3D - this.motionX);
          this.motionZ += (d1 / d5 * 0.3D - this.motionZ);
        }
      }
    }
    ...
    if (flag && this.worldObj.rand.nextInt(40) == 0)
    {
    	this.worldObj.newExplosion(this, this.posX, this.posY + (this.rand.nextFloat() * this.height), this.posZ, 1.0F, false, false);
    }
    ...
    for (int j = 0; j < 3; j++)
    {
      ...
      this.worldObj.spawnParticle(EnumParticleTypes.BLOCK_CRACK, d10 + this.rand.nextGaussian() * 0.30000001192092896D, d2 + this.rand.nextGaussian() * 0.30000001192092896D, d4 + this.rand.nextGaussian() * 0.3D, 0.0D, 0.0D, 0.0D, new int[] { 7 });
      
      this.worldObj.spawnParticle(EnumParticleTypes.BLOCK_CRACK, this.posX + (this.rand.nextDouble() - 0.5D) * this.width, this.posY + this.rand.nextDouble() * this.height, this.posZ + (this.rand.nextDouble() - 0.5D) * this.width, 0.0D, 0.0D, 0.0D, new int[] { 7 });
    }
    if (getInvulTime() > 0) {
      for (int j = 0; j < 15; j++) {
  ...
  protected void updateAITasks()
  {
    ...
    {
      super.updateAITasks();
      if (this.blockBreakCounter <= 0 && this.getAttackTarget() != null) {
        ...
        if (this.ticksExisted >= this.field_82223_h[(i - 1)])
        {
          ...
          if (i1 > 0)
          {
            ...
            if ((entity != null) && (entity.isEntityAlive()) && (getDistanceSqToEntity(entity) <= 900.0D) && (canEntityBeSeen(entity)))
            {
              ...
              this.field_82223_h[(i - 1)] = (this.ticksExisted + 160);
              this.field_82224_i[(i - 1)] = 0;
            }
            else
            {
          ...
          {
            List list = this.worldObj.getEntitiesWithinAABB(EntityLivingBase.class, getEntityBoundingBox().expand(30.0D, 30.0D, 30.0D), Predicates.and(attackEntitySelector, IEntitySelector.NOT_SPECTATING));
  ...
  {
    return p_82208_1_ <= 0 ? this.posY + 2.2D : this.posY + 1.3D;
  }
  ...
  private void launchWitherSkullToCoords(int p_82209_1_, double p_82209_2_, double p_82209_4_, double p_82209_6_, boolean p_82209_8_)
  {
    ...
    EntityBlockSkull entitywitherskull = new EntityBlockSkull(this.worldObj, this, d6, d7, d8);
    if (p_82209_8_) {
  ...
  public boolean attackEntityFrom(DamageSource source, float amount)
  {
    ...
    if (!(source.isExplosion()) && (source != DamageSource.fall) && (source != DamageSource.anvil) && (source != DamageSource.fallingBlock) && (source != DamageSource.drown) && (source != DamageSource.inWall) && (!(source.getEntity() instanceof EntityObsidianWither)))
    {
      ...
      if ((!source.isMagicDamage()) && ((source.getSourceOfDamage() instanceof EntityLivingBase)) && amount <= 500F)
      {
        ...
        if (!source.isExplosion()) {
        	...
        	entitylivingbase.attackEntityFrom(DamageSource.outOfWorld, 1F);
          entitylivingbase.attackEntityFrom(source, amount);
          double d1 = this.posX - entitylivingbase.posX;
          double d0;

          for (d0 = this.posZ - entitylivingbase.posZ; d1 * d1 + d0 * d0 < 1.0E-4D; d0 = (Math.random() - Math.random()) * 0.01D)
          {
              d1 = (Math.random() - Math.random()) * 0.01D;
          }
          entitylivingbase.attackedAtYaw = (float)(Math.atan2(d0, d1) * 180.0D / Math.PI - (double)entitylivingbase.rotationYaw);
          entitylivingbase.knockBack(this, amount * 10F, d1, d0);
        }
      }
      if ((entity != null) && !(entity instanceof IBossDisplayData)) {
        return false;
      }
      if (this.blockBreakCounter <= 0) {
        this.blockBreakCounter = 1;
      ...
      for (int i = 0; i < this.field_82223_h.length; i++) {
          this.field_82223_h[i] += 15;
        }
