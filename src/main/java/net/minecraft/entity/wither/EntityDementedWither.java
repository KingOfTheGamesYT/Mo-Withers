...
public class EntityDementedWither
  extends EntityMob
  implements IRangedAttackMob
{
  private float[] field_82220_d = new float[4];
  private float[] field_82221_e = new float[4];
  private float[] field_82217_f = new float[4];
  private float[] field_82218_g = new float[4];
  private int[] field_82223_h = new int[4];
  private int[] field_82224_i = new int[4];
  ...
  public EntityDementedWither(World worldIn)
  {
    super(worldIn);
  ...
  public void onLivingUpdate()
  {
    ...
    for (int j = 0; j < 5; j++)
    {
      ...
      this.worldObj.spawnParticle(EnumParticleTypes.SMOKE_LARGE, this.posX + this.rand.nextGaussian() * 0.5D, this.posY + this.getEyeHeight() + this.rand.nextGaussian() * 0.5D, this.posZ + this.rand.nextGaussian() * 0.5D, 0.0D, 0.0D, 0.0D, new int[0]);
      ...
      for (int i = 1; i < 5; i++) {
        if (this.ticksExisted >= this.field_82223_h[(i - 1)])
        {
          ...
          if (i1 > 0)
          {
            ...
            if ((entity != null) && (entity.isEntityAlive()) && (getDistanceSqToEntity(entity) <= 2000.0D))
            {
              ...
              if (canEntityBeSeen(entity)) {
                ...
                this.field_82223_h[(i - 1)] = (this.ticksExisted + 5);
              }
              this.field_82224_i[(i - 1)] = 0;
            }
            else
          ...
          {
            List list = this.worldObj.getEntitiesWithinAABB(EntityLivingBase.class, getEntityBoundingBox().expand(32.0D, 12.0D, 32.0D), Predicates.and(attackEntitySelector, IEntitySelector.NOT_SPECTATING));
            for (int k1 = 0; (k1 < 40) && (!list.isEmpty()); k1++)
            {
  ...
  {
    return p_82208_1_ >= 3 ? this.posY + (this.getEyeHeight() * 0.9F) : p_82208_1_ <= 0 ? this.posY + this.getEyeHeight() : this.posY + (this.getEyeHeight() * 0.6F);
  }
  ...
  private void launchWitherSkullToEntity(int p_82216_1_, EntityLivingBase p_82216_2_)
  {
    if (canEntityBeSeen(p_82216_2_)) {
      ...
      launchWitherSkullToCoords(p_82216_1_, p_82216_2_.posX, p_82216_2_.posY + p_82216_2_.getEyeHeight() * 0.5D, p_82216_2_.posZ, true);
    }
  }
  ...
  public boolean attackEntityFrom(DamageSource source, float amount)
  {
    ...
    if ((source != DamageSource.fall) && (source != DamageSource.anvil) && (source != DamageSource.fallingBlock) && (source != DamageSource.drown) && (source != DamageSource.inWall) && (!(source.getEntity() instanceof EntityDementedWither)))
    {
  ...
  {
    dropItem(Items.bone, 128);
    
    EntityItem entityitem = dropItem(Items.nether_star, 2 + this.rand.nextInt(3));
    if (entityitem != null) {
  ...
  {
    this.dataWatcher.updateObject(22, Integer.valueOf(p_82215_1_));
  }
