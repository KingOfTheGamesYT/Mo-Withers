...
public class EntityLightningWither
  extends EntityMob
  implements IRangedAttackMob
{
  ...
  public EntityLightningWither(World worldIn)
  {
    super(worldIn);
    setAbsorptionAmount(getMaxHealth() / 4.0F);
    setHealth(getMaxHealth());
    setSize(0.9F, 3.5F);
    this.isImmuneToFire = true;
    ((PathNavigateGround)getNavigator()).func_179693_d(true);
    this.tasks.addTask(0, new EntityAISwimming(this));
    this.tasks.addTask(2, new EntityAIAttackRanged(this, 1.0D, 20, 100.0F));
  ...
  {
    return "mowithers:LightningWitherIdle";
  }
  
  protected String getHurtSound()
  {
    return "mowithers:LightningWitherHurt";
  }
  
  protected String getDeathSound()
  {
    return "mowithers:LightningWitherDeath";
  }
  ...
  public void onLivingUpdate()
  {
    ...
    if ((!this.worldObj.isRemote) && (getWatchedTargetId(0) > 0))
    {
      ...
      if (entity != null)
      {
        if ((this.posY < entity.posY) || ((!isArmored()) && (this.posY < entity.posY + 6.0D)))
        {
          ...
          this.motionY += (0.8D - this.motionY) * 0.8D;
        }
    ...
    for (int j = 0; j < 3; j++)
    {
      ...
      this.worldObj.spawnParticle(EnumParticleTypes.CRIT, d10 + this.rand.nextGaussian() * 0.30000001192092896D, d2 + this.rand.nextGaussian() * 0.30000001192092896D, d4 + this.rand.nextGaussian() * 0.3D, 0.0D, 0.75D, 0.0D, new int[0]);
      if ((!isInWater()) && (isWet()))
      {
        this.worldObj.spawnParticle(EnumParticleTypes.CLOUD, this.posX + (this.rand.nextDouble() - 0.5D) * this.width * 3.0D, d2 + this.rand.nextGaussian() * 0.30000001192092896D + 12.0D, this.posZ + (this.rand.nextDouble() - 0.5D) * this.width * 3.0D, 0.0D, 0.0D, 0.0D, new int[0]);
        this.worldObj.spawnParticle(EnumParticleTypes.CLOUD, this.posX + (this.rand.nextDouble() - 0.5D) * this.width * 3.0D, d2 + this.rand.nextGaussian() * 0.30000001192092896D + 12.0D, this.posZ + (this.rand.nextDouble() - 0.5D) * this.width * 3.0D, 0.0D, 0.0D, 0.0D, new int[0]);
        this.worldObj.spawnParticle(EnumParticleTypes.CLOUD, this.posX + (this.rand.nextDouble() - 0.5D) * this.width * 3.0D, d2 + this.rand.nextGaussian() * 0.30000001192092896D + 12.0D, this.posZ + (this.rand.nextDouble() - 0.5D) * this.width * 3.0D, 0.0D, 0.0D, 0.0D, new int[0]);
        this.worldObj.spawnParticle(EnumParticleTypes.CLOUD, this.posX + (this.rand.nextDouble() - 0.5D) * this.width * 3.0D, d2 + this.rand.nextGaussian() * 0.30000001192092896D + 12.0D, this.posZ + (this.rand.nextDouble() - 0.5D) * this.width * 3.0D, 0.0D, 0.0D, 0.0D, new int[0]);
        
        this.worldObj.spawnParticle(EnumParticleTypes.FIREWORKS_SPARK, this.posX + (this.rand.nextDouble() - 0.5D) * this.width, d2 + this.rand.nextGaussian() * 0.30000001192092896D + 12.0D, this.posZ + (this.rand.nextDouble() - 0.5D) * this.width, 0.0D, -0.85D, 0.0D, new int[0]);
        this.worldObj.spawnParticle(EnumParticleTypes.FIREWORKS_SPARK, this.posX + (this.rand.nextDouble() - 0.5D) * this.width, d2 + this.rand.nextGaussian() * 0.30000001192092896D + 12.0D, this.posZ + (this.rand.nextDouble() - 0.5D) * this.width, 0.0D, -0.85D, 0.0D, new int[0]);
      }
      if (this.worldObj.rand.nextInt(10) == 0)
      {
        this.worldObj.spawnParticle(EnumParticleTypes.FIREWORKS_SPARK, d10 + this.rand.nextGaussian() * 0.30000001192092896D, d2 + this.rand.nextGaussian() * 0.30000001192092896D, d4 + this.rand.nextGaussian() * 0.6D, 0.0D, 0.75D, 0.0D, new int[0]);
        this.worldObj.spawnParticle(EnumParticleTypes.FIREWORKS_SPARK, d10 + this.rand.nextGaussian() * 0.30000001192092896D, d2 + this.rand.nextGaussian() * 0.30000001192092896D, d4 + this.rand.nextGaussian() * 0.6D, 0.0D, 0.0D, 0.0D, new int[0]);
      }
    ...
    if (getInvulTime() > 0) {
      for (int j = 0; j < 15; j++) {
        this.worldObj.spawnParticle(EnumParticleTypes.CLOUD, this.posX + this.rand.nextGaussian() * 1.0D, this.posY + this.rand.nextFloat() * 3.3F, this.posZ + this.rand.nextGaussian() * 0.6D, 0.0D, 0.0D, 0.0D, new int[0]);
      }
    }
  }
  ...
  protected void updateAITasks()
  {
    ...
    else
    {
      ...
      if ((!this.worldObj.isRemote) && (this.worldObj.isThundering()) && (this.worldObj.canBlockSeeSky(new BlockPos(this))) && (this.rand.nextInt(40) == 0)) {
          this.worldObj.addWeatherEffect(new EntityLightningBolt(this.worldObj, this.posX - 0.5D, this.posY + 3.3499999046325684D, this.posZ - 0.5D));
        }
      for (int i = 1; i < 3; i++) {
        if (this.ticksExisted >= this.field_82223_h[(i - 1)])
        {
          ...
          if (i1 > 0)
          {
            ...
            if ((entity != null) && (entity.isEntityAlive()) && (getDistanceSqToEntity(entity) <= 10000.0D) && (canEntityBeSeen(entity)))
            {
              ...
              this.field_82223_h[(i - 1)] = (this.ticksExisted + 10 + this.rand.nextInt(30));
              this.field_82224_i[(i - 1)] = 0;
            }
            else
            {
      ...
      if (this.ticksExisted % 20 == 0) {
        if ((getHealth() == getMaxHealth()) && (getAbsorptionAmount() < getMaxHealth() / 4.0F)) {
  ...
  private void launchWitherSkullToCoords(int p_82209_1_, double p_82209_2_, double p_82209_4_, double p_82209_6_, boolean p_82209_8_)
  {
    ...
    EntityLightningSkull entitywitherskull = new EntityLightningSkull(this.worldObj, this, d6, d7, d8);
    if (p_82209_8_) {
  ...
  {
    if (isEntityInvulnerable(source) || this.hurtResistantTime > 0) {
      return false;
    }
    if (source == DamageSource.lightningBolt)
    {
      heal(amount);
      if (getAbsorptionAmount() < 300.0F) {
        setAbsorptionAmount(getAbsorptionAmount() + amount);
      }
      return false;
    }
    if ((source != DamageSource.lava) && (source != DamageSource.inFire) && (source != DamageSource.onFire) && (!(source.getEntity() instanceof EntityLightningWither)))
    {
      ...
      if (getAbsorptionAmount() > 0.0F)
      {
        amount /= 2.0F;
        playSound("random.anvil_land", 3.0F, getSoundPitch() - this.rand.nextFloat() * 0.7F);
  ...
  protected void dropFewItems(boolean p_70628_1_, int p_70628_2_)
  {
    ...
    if (!this.worldObj.isRemote)
    {
        ...
        while (iterator.hasNext())
        {
            ...
            entityplayer.triggerAchievement(MoWithersAchievments.achievementKillLightningWither);
        }
    }
  }
