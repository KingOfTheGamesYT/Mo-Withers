  ...
  implements IBossDisplayData, IRangedAttackMob
{
  ...
  public EntityVoidWither(World worldIn)
  {
    super(worldIn);
    ...
    this.tasks.addTask(2, new EntityAIAttackRanged(this, 1.0D, 10, 100.0F));
  ...
  public void onLivingUpdate()
  {
    ...
    for (int j = 0; j < 15; j++)
    {
      ...
      this.worldObj.spawnParticle(EnumParticleTypes.SUSPENDED_DEPTH, d10 + this.rand.nextGaussian() * 0.30000001192092896D, d2 + this.rand.nextGaussian() * 0.30000001192092896D, d4 + this.rand.nextGaussian() * 0.3D, 0.0D, 0.0D, 0.0D, new int[0]);
  ...
  protected void updateAITasks()
  {
    if (getInvulTime() > 0)
    {
      ...
      if (i <= 0)
      {
        this.worldObj.newExplosion(this, this.posX, this.posY + getEyeHeight(), this.posZ, 14.0F, false, this.worldObj.getGameRules().getGameRuleBooleanValue("mobGriefing"));
    ...
    else
    {
      ...
      List list1 = this.worldObj.getEntitiesWithinAABB(EntityBlackHole.class, getEntityBoundingBox().expand(100.0D, 100.0D, 100.0D));
      if (list1 != null && !list1.isEmpty()) {
        for (int i2 = 0; i2 < list1.size(); i2++)
        {
        	float f6 = this.ticksExisted * (float)Math.PI * 0.015F + (i2 * i2);
          Entity entity = (Entity)list1.get(i2);
          
          if (entity != null) 
          {
        	  if (((EntityBlackHole)entity).shootingEntity != this)
        	  ((EntityBlackHole)entity).shootingEntity = this;
              double d2 = (this.posX + (MathHelper.cos(f6) * 12.0F)) - entity.posX;
              double d3 = (this.posY + 10D) - entity.posY;
              double d4 = (this.posZ + (MathHelper.sin(f6) * 12.0F)) - entity.posZ;
      ...
      for (int i = 1; i < 3; i++) {
        ...
        {
          this.field_82223_h[(i - 1)] = (this.ticksExisted + 2 + this.rand.nextInt(2));
          ...
          {
            List list = this.worldObj.getEntitiesWithinAABB(EntityLivingBase.class, getEntityBoundingBox().expand(100.0D, 8.0D, 100.0D), Predicates.and(attackEntitySelector, IEntitySelector.NOT_SPECTATING));
  ...
  private double func_82214_u(int p_82214_1_)
  {
    ...
    return this.posX + f1 * 2.6D;
  }
  
  private double func_82208_v(int p_82208_1_)
  {
    return p_82208_1_ <= 0 ? this.posY + 6.0D : this.posY + 4.4D;
  }
  ...
  {
    playSound("mob.wither.shoot", 2.0F, 0.75F);
  ...
  public boolean attackEntityFrom(DamageSource source, float amount)
  {
    ...
    if ((source != DamageSource.outOfWorld) && (source != DamageSource.lava) && (source != DamageSource.inFire) && (source != DamageSource.onFire) && (!(source.getEntity() instanceof EntityVoidWither)))
    {
      if (getInvulTime() > 0) {
        return false;
      }
