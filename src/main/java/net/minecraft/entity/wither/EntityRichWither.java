  ...
  implements IRangedAttackMob
{
  ...
  private static final Predicate attackEntitySelector = new Predicate()
  {
    ...
    {
      return (((p_180027_1_ instanceof EntityLivingBase)) && (((EntityLivingBase)p_180027_1_).getCreatureAttribute() != EnumCreatureAttribute.UNDEAD) && !(p_180027_1_ instanceof EntityVillager));
    }
  ...
  public EntityRichWither(World worldIn)
  {
    super(worldIn);
  ...
  public void writeEntityToNBT(NBTTagCompound tagCompound)
  {
    ...
    tagCompound.setInteger("Money", getMoney());
  }
  ...
  public void readEntityFromNBT(NBTTagCompound tagCompund)
  {
    ...
    setMoney(tagCompund.getInteger("Money"));
  }
  ...
  public void onLivingUpdate()
  {
    ...
    if ((!this.worldObj.isRemote) && (getWatchedTargetId(0) > 0))
    {
      ...
      if (this.isEntityAlive() && entity != null)
      {
    ...
    for (int j = 0; j < 3; j++)
    {
      ...
      this.worldObj.spawnParticle(EnumParticleTypes.BLOCK_CRACK, d10 + this.rand.nextGaussian() * 0.30000001192092896D, d2 + this.rand.nextGaussian() * 0.30000001192092896D, d4 + this.rand.nextGaussian() * 0.3D, 0.0D, 0.0D, 0.0D, new int[] { 41 });
      
      this.worldObj.spawnParticle(EnumParticleTypes.BLOCK_CRACK, this.posX + (this.rand.nextDouble() - 0.5D) * this.width, this.posY + this.rand.nextDouble() * this.height, this.posZ + (this.rand.nextDouble() - 0.5D) * this.width, 0.0D, 0.0D, 0.0D, new int[] { 41 });
    }
    if (getInvulTime() > 0) {
      for (int j = 0; j < 15; j++) {
    ...
    else
    {
      ...
      if ((list111 != null) && (!list111.isEmpty())) {
        for (int i111 = 0; i111 < list111.size(); i111++)
        {
          ...
          if (this.rand.nextInt(20) == 0 && (entity != null) && ((entity instanceof EntityVillager))) 
          {
        	((EntityVillager)entity).faceEntity(this, 180F, 30F);
            this.setMoney(this.getMoney() + 1);
            this.worldObj.playSoundEffect(entity.posX, entity.posY, entity.posZ, "mowithers:WeHaveLosers", 1.0F, 1.5F);

            EntityItem item = new EntityItem(worldObj, entity.posX, entity.posY, entity.posZ, new ItemStack(Items.gold_ingot));
            double d0 = this.posX - item.posX;
            double d1 = this.posX - item.posX;
            double d3 = d0 * d0 + d1 * d1;
            double d5 = MathHelper.sqrt_double(d3);
            item.addVelocity(d0 / d5 * 0.5D, 0.25D, d1 / d5 * 0.5D);
      ...
      for (int i = 1; i < 3; i++) {
        if (this.ticksExisted >= this.field_82223_h[(i - 1)])
        {
          ...
          {
            List list = this.worldObj.getEntitiesWithinAABB(EntityLivingBase.class, getEntityBoundingBox().expand(32.0D, 32.0D, 32.0D), Predicates.and(attackEntitySelector, IEntitySelector.NOT_SPECTATING));
            for (int k1 = 0; (k1 < 5) && (!list.isEmpty()); k1++)
            {
  ...
  {
    this.worldObj.playSoundEffect(this.posX, this.posY, this.posZ, "mowithers:ImReallyRich", 4.0F, 1.0F);
    ...
    if (p_82209_8_) 
    {
      entitywitherskull.setSkullType(2 + rand.nextInt(1));
    }
    else
    {
        entitywitherskull.setSkullType(0 + rand.nextInt(1));
    }
  ...
  public boolean attackEntityFrom(DamageSource source, float amount)
  {
    ...
    if ((source != DamageSource.fall) && (source != DamageSource.anvil) && (source != DamageSource.fallingBlock) && (source != DamageSource.drown) && (source != DamageSource.inWall) && (!(source.getEntity() instanceof EntityRichWither)))
    {
      ...
      if ((float)this.hurtResistantTime <= (float)this.maxHurtResistantTime / 2.0F)
      {
    	  if (entity != null)
    	  {
    		  this.getLookHelper().setLookPositionWithEntity(entity, 180F, 30F);
    	  }
          if (!this.worldObj.isRemote && entity != null)
              entity.dropItem(Items.gold_ingot, 1);
          	  this.setMoney(this.getMoney() - 1);
              this.worldObj.playSoundEffect(this.posX, this.posY, this.posZ, "mowithers:GetTheHellOuttaHere", 4.0F, 1.0F);
      }
  ...
  public void setDead()
  {
	  ...
	  if (!this.worldObj.isRemote)
	  {
		  if (this.getMoney() > 0)
		  {
			  EntityRichWither wither = new EntityRichWither(this.worldObj);
			  wither.copyLocationAndAnglesFrom(this);
			  wither.setMoney(this.getMoney() - 50000);
			  this.worldObj.spawnEntityInWorld(wither);
			  dropItem(Items.gold_ingot, 32);
			  if (this.getAttackTarget() != null)
				  wither.setAttackTarget(this.getAttackTarget());
		  }
		  else
		  {
			  this.worldObj.playSoundEffect(this.posX, this.posY, this.posZ, "mowithers:WeHaveLosers", 4.0F, 0.9F);
			  this.playSound(getDeathSound(), getSoundVolume(), getSoundPitch() * 0.2F);
  ...
  public void setMoney(int p_82215_1_)
  {
    this.dataWatcher.updateObject(21, Integer.valueOf(p_82215_1_));
  }
  
  public void earnMoney(int p_82215_1_)
  {
    this.setMoney(this.getMoney() + p_82215_1_);
  }
  
  public void spendMoney(int p_82215_1_)
  {
    this.setMoney(this.getMoney() - p_82215_1_);
  }
