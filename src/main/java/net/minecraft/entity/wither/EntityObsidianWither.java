...
public class EntityObsidianWither
  extends EntityMob
  implements IRangedAttackMob
{
  ...
  public EntityObsidianWither(World worldIn)
  {
    super(worldIn);
    ...
    this.tasks.addTask(2, new EntityAIAttackRanged(this, 1.0D, 80, 20.0F));
  ...
  public void onLivingUpdate()
  {
    ...
    if ((!this.worldObj.isRemote) && (getWatchedTargetId(0) > 0))
    {
      ...
      if (entity != null)
      {
        ...
        if (d3 > 9.0D)
        {
          ...
          this.motionX += (d0 / d5 * 0.4D - this.motionX) * 0.4D;
          this.motionZ += (d1 / d5 * 0.4D - this.motionZ) * 0.4D;
        }
      }
    }
    ...
    for (int j = 0; j < 3; j++)
    {
      ...
      this.worldObj.spawnParticle(EnumParticleTypes.BLOCK_CRACK, d10 + this.rand.nextGaussian() * 0.30000001192092896D, d2 + this.rand.nextGaussian() * 0.30000001192092896D, d4 + this.rand.nextGaussian() * 0.3D, 0.0D, 0.0D, 0.0D, new int[] { 49 });
      
      this.worldObj.spawnParticle(EnumParticleTypes.BLOCK_CRACK, this.posX + (this.rand.nextDouble() - 0.5D) * this.width, this.posY + this.rand.nextDouble() * this.height, this.posZ + (this.rand.nextDouble() - 0.5D) * this.width, 0.0D, 0.0D, 0.0D, new int[] { 49 });
    }
    if (getInvulTime() > 0) {
      for (int j = 0; j < 15; j++) {
        ...
        if (this.ticksExisted >= this.field_82223_h[(i - 1)])
        {
          ...
          if (i1 > 0)
          {
            ...
            if ((entity != null) && (entity.isEntityAlive()) && (getDistanceSqToEntity(entity) <= 900.0D) && (canEntityBeSeen(entity)))
            {
              ...
              this.field_82223_h[(i - 1)] = (this.ticksExisted + 120);
              this.field_82224_i[(i - 1)] = 0;
            }
            else
            {
  ...
  {
    return (block != Blocks.ender_chest) && (block != Blocks.obsidian) && (block != Blocks.barrier) && (block != Blocks.bedrock) && (block != Blocks.end_portal) && (block != Blocks.end_portal_frame) && (block != Blocks.command_block);
  }
  ...
  private void launchWitherSkullToCoords(int p_82209_1_, double p_82209_2_, double p_82209_4_, double p_82209_6_, boolean p_82209_8_)
  {
    ...
    entitywitherskull.setSkullType(3);
  ...
  protected void applyEntityAttributes()
  {
    ...
    getEntityAttribute(SharedMonsterAttributes.maxHealth).setBaseValue(7500.0D);
  ...
  {
    return this.isChild() ? (this.rand.nextFloat() - this.rand.nextFloat()) * 0.1F + 1.25F : (this.rand.nextFloat() - this.rand.nextFloat()) * 0.1F + 0.75F;
  }
}
