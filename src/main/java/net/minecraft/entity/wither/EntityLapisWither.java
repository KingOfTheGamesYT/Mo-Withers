...
public class EntityLapisWither
  extends EntityMob
  implements IRangedAttackMob
{
  ...
  public EntityLapisWither(World worldIn)
  {
    super(worldIn);
  ...
  public void onLivingUpdate()
  {
    ...
    for (int j = 0; j < 3; j++)
    {
      ...
      if ((flag) && (this.worldObj.rand.nextInt(4) == 0)) {
        this.worldObj.spawnParticle(EnumParticleTypes.BLOCK_CRACK, d10 + this.rand.nextGaussian() * 0.30000001192092896D, d2 + this.rand.nextGaussian() * 0.30000001192092896D, d4 + this.rand.nextGaussian() * 0.30000001192092896D, 0.699999988079071D, 0.699999988079071D, 0.5D, new int[] {22});
      }
    }
    if (getInvulTime() > 0) {
      for (int j = 0; j < 15; j++) {
        this.worldObj.spawnParticle(EnumParticleTypes.WATER_DROP, this.posX + this.rand.nextGaussian() * 1.0D, this.posY + this.rand.nextFloat() * 3.3F, this.posZ + this.rand.nextGaussian() * 0.6D, 0.0D, 0.0D, 0.0D, new int[] {42});
      }
    }
  }
  ...
  {
    return (block != Blocks.enchanting_table) && (block != Blocks.lapis_block) && (block != Blocks.lapis_ore) && (block != Blocks.barrier) && (block != Blocks.bedrock) && (block != Blocks.end_portal) && (block != Blocks.end_portal_frame) && (block != Blocks.command_block);
  }
  ...
  private void launchWitherSkullToCoords(int p_82209_1_, double p_82209_2_, double p_82209_4_, double p_82209_6_, boolean p_82209_8_)
  {
    ...
    entitywitherskull.setSkullType(6);
  ...
  public boolean attackEntityFrom(DamageSource source, float amount)
  {
    ...
    if ((source != DamageSource.fall) && (source != DamageSource.anvil) && (source != DamageSource.fallingBlock) && (source != DamageSource.drown) && (source != DamageSource.inWall) && (!(source.getEntity() instanceof EntityLapisWither)))
    {
  ...
  {
	  this.entityDropItem(new ItemStack(Items.dye, 128, EnumDyeColor.BLUE.getDyeDamage()), 0.0F);
