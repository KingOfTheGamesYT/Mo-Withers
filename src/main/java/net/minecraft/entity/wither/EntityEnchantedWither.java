...
public class EntityEnchantedWither
  extends EntityGolem
  implements IRangedAttackMob
{
  ...
  public EntityEnchantedWither(World worldIn)
  {
    super(worldIn);
    ...
    this.tasks.addTask(2, new EntityAIAttackRanged(this, 1.0D, 1, 40, 20.0F));
  ...
  {
	  this.playSound("mob.wither.idle", this.getSoundVolume(), this.getSoundPitch() - 0.1F);
    return "mob.wither.idle";
  }
  
  protected String getHurtSound()
  {
	  this.playSound("mob.wither.hurt", this.getSoundVolume(), this.getSoundPitch() - 0.1F);
    return "mob.wither.hurt";
  }
  
  protected String getDeathSound()
  {
	  this.playSound("mob.wither.death", this.getSoundVolume(), this.getSoundPitch() - 0.1F);
  ...
  public void onUpdate()
  {
      ...
      EntityPlayer entityplayer = this.worldObj.getClosestPlayer(this.posX, this.posY + 3D, this.posZ, 5.0D);
    ...
    else
    {
      ...
      for (int i = 1; i < 3; i++) {
        if (this.ticksExisted >= this.field_82223_h[(i - 1)])
        {
          ...
          {
            List list = this.worldObj.getEntitiesWithinAABB(EntityLivingBase.class, getEntityBoundingBox().expand(20.0D, 8.0D, 20.0D), Predicates.and(IMob.mobSelector, IEntitySelector.NOT_SPECTATING));
  ...
  public boolean interact(EntityPlayer player)
  {
    ...
    if ((itemstack != null) && !itemstack.isItemEnchanted() && (itemstack.getItem() instanceof ItemBow || itemstack.getItem() instanceof ItemFishingRod || itemstack.getItem() instanceof ItemSword || itemstack.getItem() instanceof ItemTool || itemstack.getItem() instanceof ItemArmor || itemstack.getItem() == Items.book))
    {
    	this.playSound("mowithers:Augment", 5F, 1F);
      if (!this.worldObj.isRemote)
      {
    	  EnchantmentHelper.addRandomEnchantment(getRNG(), itemstack, 16);
    	  EnchantmentHelper.addRandomEnchantment(getRNG(), itemstack, 18);
    	  EnchantmentHelper.addRandomEnchantment(getRNG(), itemstack, 22);
    	  EnchantmentHelper.addRandomEnchantment(getRNG(), itemstack, 26);
    	  EnchantmentHelper.addRandomEnchantment(getRNG(), itemstack, 30);
    	  EnchantmentHelper.addRandomEnchantment(getRNG(), itemstack, 34);
    	  EnchantmentHelper.addRandomEnchantment(getRNG(), itemstack, 38);
    	  EnchantmentHelper.addRandomEnchantment(getRNG(), itemstack, 42);
      }
      player.swingItem();
  ...
  public boolean attackEntityFrom(DamageSource source, float amount)
  {
    ...
    if ((source != DamageSource.magic) && (!source.isMagicDamage()) && (source != DamageSource.lightningBolt) && (!(source.getEntity() instanceof EntityEnchantedWither)))
    {
