  ...
  implements IRangedAttackMob
{
  ...
  public EntityBabyWither(World worldIn)
  {
    ...
    this.tasks.addTask(0, new EntityAIAvoidEntity(this, new Predicate()
    {
      ...
      {
        return p_179958_1_ instanceof EntityPlayer;
      }
    ...
    this.tasks.addTask(3, new EntityAIWatchClosest2(this, EntityWither.class, 4.0F, 1.0F));
  ...
  public void writeEntityToNBT(NBTTagCompound tagCompound)
  {
    ...
    tagCompound.setInteger("Growth", getGrowthPoints());
  }
  ...
  public void readEntityFromNBT(NBTTagCompound tagCompund)
  {
    ...
    setGrowthPoints(tagCompund.getInteger("Growth"));
  }
  ...
  public void onLivingUpdate()
  {
    ...
    if (!this.worldObj.isRemote && this.getGrowthPoints() >= 7500)
    {
    	EntityWither wither = new EntityWither(worldObj);
    	wither.copyLocationAndAnglesFrom(this);
    	this.setDead();
    	worldObj.spawnEntityInWorld(wither);
    	wither.setInvulTime(400);
    	wither.setHealth(this.getMaxHealth() / 5.0F);
    }
    ...
    if ((!this.worldObj.isRemote) && (getWatchedTargetId(0) > 0))
    {
      ...
      if (entity != null)
      {
        if ((this.posY < entity.posY) || ((!isArmored()) && (this.posY < entity.posY + 9.0D)))
        {
          if (this.motionY < 0.0D) {
            this.motionY = 0.0D;
          }
          this.motionY += (0.75D - this.motionY) * 0.75D;
        }
        double d0 = entity.posX - this.posX;
        double d1 = entity.posZ - this.posZ;
        double d3 = d0 * d0 + d1 * d1;
        if ((d3 > 12.0D) && (!isArmored()))
        {
          double d5 = MathHelper.sqrt_double(d3);
          this.motionX += (d0 / d5 * 0.75D - this.motionX) * 0.75D;
          this.motionZ += (d1 / d5 * 0.75D - this.motionZ) * 0.75D;
        }
      }
    }
    ...
    for (i = 0; i < 2; i++)
    {
      ...
      {
        this.field_82221_e[i] = func_82204_b(this.field_82221_e[i], this.renderYawOffset, 180.0F);
      }
    }
  ...
  protected void updateAITasks()
  {
    if (getInvulTime() > 0)
    {
      ...
      if (i <= 0)
      {
        this.worldObj.newExplosion(this, this.posX, this.posY + getEyeHeight(), this.posZ, 3.0F, false, this.worldObj.getGameRules().getGameRuleBooleanValue("mobGriefing"));
    ...
    else
    {
        ...
        if (this.ticksExisted >= this.field_82223_h[(i - 1)])
        {
          ...
          if (i1 > 0)
          {
            ...
            if ((entity != null) && (entity.isEntityAlive()) && (getDistanceSqToEntity(entity) <= 900.0D) && (canEntityBeSeen(entity)))
            {
              ...
              this.field_82223_h[(i - 1)] = (this.ticksExisted + 80);
              this.field_82224_i[(i - 1)] = 0;
            }
            else
            {
      ...
      if (this.ticksExisted % 20 == 0) {
        ...
        if (this.rand.nextInt(2) == 0)
        	this.setGrowthPoints(this.getGrowthPoints() + 1);
      }
    }
  }
  ...
  private double func_82214_u(int p_82214_1_)
  {
    ...
    return this.posX + f1 * 0.75D;
  }
  
  private double func_82208_v(int p_82208_1_)
  {
    return p_82208_1_ <= 0 ? this.posY + 1.1D : this.posY + 1.0D;
  }
  ...
  {
    playSound("mob.wither.shoot", 2.0F, 2.0F);
  ...
  public boolean attackEntityFrom(DamageSource source, float amount)
  {
    ...
    if ((source != DamageSource.drown) && (!(source.getEntity() instanceof EntityBabyWither)))
    {
  ...
  public void onKillEntity(EntityLivingBase entityLivingIn)
  {
      ...
      this.setGrowthPoints((int)(this.getGrowthPoints() + 5 + entityLivingIn.width + entityLivingIn.height));
  }
  ...
  public void setGrowthPoints(int p_82215_1_)
  {
