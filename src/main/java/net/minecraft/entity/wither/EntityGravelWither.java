...
public class EntityGravelWither
  extends EntityMob
  implements IRangedAttackMob
{
  ...
  public EntityGravelWither(World worldIn)
  {
    super(worldIn);
  ...
  {
    return "mowithers:SandWitherIdle";
  }
  
  protected String getHurtSound()
  {
    return "mowithers:SandWitherHurt";
  }
  
  protected String getDeathSound()
  {
    return "mowithers:SandWitherDeath";
  }
  ...
  public void onLivingUpdate()
  {
    ...
    for (int j = 0; j < 3; j++)
    {
      ...
      this.worldObj.spawnParticle(EnumParticleTypes.BLOCK_CRACK, d10 + this.rand.nextGaussian() * 0.30000001192092896D, d2 + this.rand.nextGaussian() * 0.30000001192092896D, d4 + this.rand.nextGaussian() * 0.3D, 0.0D, 0.0D, 0.0D, new int[] { 13 });
      
      this.worldObj.spawnParticle(EnumParticleTypes.BLOCK_CRACK, this.posX + (this.rand.nextDouble() - 0.5D) * this.width, this.posY + this.rand.nextDouble() * this.height, this.posZ + (this.rand.nextDouble() - 0.5D) * this.width, 0.0D, 0.0D, 0.0D, new int[] { 13 });
    }
    if (getInvulTime() > 0) {
      for (int j = 0; j < 15; j++) {
  ...
  public boolean attackEntityFrom(DamageSource source, float amount)
  {
    ...
    if ((source != DamageSource.fall) && (source != DamageSource.anvil) && (source != DamageSource.fallingBlock) && (source != DamageSource.drown) && (source != DamageSource.inWall) && (!(source.getEntity() instanceof EntityGravelWither)))
    {
