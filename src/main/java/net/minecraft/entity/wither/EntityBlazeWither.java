...
public class EntityBlazeWither
  extends EntityMob
  implements IRangedAttackMob
{
  ...
  private static final Predicate attackEntitySelector = new Predicate()
  {
    ...
    {
      return (p_180027_1_ instanceof EntityLivingBase && !(p_180027_1_ instanceof EntityBlaze) && !(p_180027_1_ instanceof EntityBlazeWither));
    }
  ...
  public EntityBlazeWither(World worldIn)
  {
    super(worldIn);
  ...
  {
	  this.playSound("mob.wither.idle", 0.5F, this.getSoundPitch());
    return "mob.blaze.breathe";
  }
  
  protected String getHurtSound()
  {
	  this.playSound("mob.wither.hurt", 0.5F, this.getSoundPitch());
    return "mob.blaze.hit";
  }
  
  protected String getDeathSound()
  {
	  this.playSound("mob.wither.death", 0.5F, this.getSoundPitch());
    return "mob.blaze.death";
  }
  
  public boolean isBurning()
  {
    return isArmored() || (this.worldObj.getEntityByID(getWatchedTargetId(1)) != null || this.worldObj.getEntityByID(getWatchedTargetId(2)) != null);
  }
  
  public void onLivingUpdate()
  {
	  if (this.getAttackTarget() != null && this.getDistanceSqToEntity(this.getAttackTarget()) <= 16D)
		  this.getAttackTarget().attackEntityFrom(DamageSource.causeMobDamage(this), 8F);
	  if ((!this.onGround) && (this.motionY < 0.0D))
    this.motionY *= 0.5D;
    if ((this.rand.nextInt(10) == 0) && (!isSilent())) {
      this.worldObj.playSound(this.posX + 0.5D, this.posY + 0.5D, this.posZ + 0.5D, "fire.fire", 3.0F, this.rand.nextFloat() * 0.7F + 0.3F, false);
    }
      ...
      if (entity != null)
      {
        ...
        if ((list111 != null) && (!list111.isEmpty())) {
          for (int i111 = 0; i111 < list111.size(); i111++)
          {
            ...
            if (entity11 != null && entity11 instanceof EntityBlaze && entity instanceof EntityLivingBase)
            {
              ((EntityBlaze)entity11).setAttackTarget((EntityLivingBase) entity);
            }
          }
        }
      }
    ...
    if (!this.isWet())
    {
        for (int j = 0; j < 3; j++)
        {
          ...
          {
              this.worldObj.spawnParticle(EnumParticleTypes.SMOKE_LARGE, d10 + this.rand.nextGaussian() * 0.30000001192092896D, d2 + this.rand.nextGaussian() * 0.30000001192092896D, d4 + this.rand.nextGaussian() * 0.3D, 0.0D, 0.0D, 0.0D, new int[0]);
    ...
    else
    {
        ...
        for (int j = 0; j < 3; j++)
        {
          ...
          for (int j1 = 0; j1 < 3; j1++)
          {
              this.worldObj.spawnParticle(EnumParticleTypes.CLOUD, d10 + this.rand.nextGaussian() * 0.30000001192092896D, d2 + this.rand.nextGaussian() * 0.30000001192092896D, d4 + this.rand.nextGaussian() * 0.3D, 0.0D, 0.25D, 0.0D, new int[0]);
              this.worldObj.spawnParticle(EnumParticleTypes.EXPLOSION_NORMAL, this.posX + (this.rand.nextDouble() - 0.5D) * this.width, this.posY + this.rand.nextDouble() * this.height, this.posZ + (this.rand.nextDouble() - 0.5D) * this.width, 0.0D, 0.25D, 0.0D, new int[0]);
              if ((flag) || (this.worldObj.rand.nextInt(10) == 0))
              {
                this.worldObj.spawnParticle(EnumParticleTypes.SMOKE_NORMAL, d10 + this.rand.nextGaussian() * 0.30000001192092896D, d2 + this.rand.nextGaussian() * 0.30000001192092896D, d4 + this.rand.nextGaussian() * 0.6D, 0.0D, 0.25D, 0.0D, new int[0]);
                this.worldObj.spawnParticle(EnumParticleTypes.CRIT, this.posX + (this.rand.nextDouble() - 0.5D) * this.width, this.posY + this.rand.nextDouble() * this.height, this.posZ + (this.rand.nextDouble() - 0.5D) * this.width, 0.0D, 0.25D, 0.0D, new int[0]);
              }
          }
        }
    }
  ...
  protected void updateAITasks()
  {
    ...
    else
    {
      ...
      {
      	EntityBlaze blazeminion = new EntityBlaze(this.worldObj);
      ...
      if ((isWet()) || (this.worldObj.getBiomeGenForCoords(new BlockPos(this.posX, this.posY, this.posZ)).getFloatTemperature(new BlockPos(this.posX, this.posY, this.posZ)) < 0.1F)) {
          ...
          if (i1 > 0)
          {
            ...
            if ((entity != null) && (entity.isEntityAlive()) && (getDistanceSqToEntity(entity) <= 1024.0D) && (canEntityBeSeen(entity)))
            {
              ...
              if (this.rand.nextInt(3) == 0)
            	  this.field_82223_h[(i - 1)] = (this.ticksExisted + 100);
              else
            	  this.field_82223_h[(i - 1)] = (this.ticksExisted + 6);
              this.field_82224_i[(i - 1)] = 0;
            }
            else
            {
  ...
  private void launchWitherSkullToCoords(int p_82209_1_, double p_82209_2_, double p_82209_4_, double p_82209_6_, boolean p_82209_8_)
  {
    ...
    EntityWitherSkullBlaze entitywitherskull = new EntityWitherSkullBlaze(this.worldObj, this, d6, d7, d8);
    if (p_82209_8_) {
  ...
  {
	  if (source.getSourceOfDamage() instanceof EntitySnowball && amount != 3F)
    	  amount = 3F;
    ...
    if ((source != DamageSource.lava) && (source != DamageSource.inFire) && (source != DamageSource.onFire) && (!(source.getEntity() instanceof EntityBlazeWither)))
    {
      ...
      if ((entity != null) && (!(entity instanceof EntityPlayer)) && ((entity instanceof EntityBlaze) || (entity instanceof EntityBlazeWither))) {
        return false;
      }
      if (this.blockBreakCounter <= 0) {
  ...
  {
      int j = 1 + this.rand.nextInt(4 + p_70628_2_);

      for (int k = 0; k < j + p_70628_2_; ++k)
      {
          this.dropItem(Items.blaze_rod, 1);
      }
      for (int k = 0; k < j + 2 + p_70628_2_; ++k)
      {
