...
public class EntityThrowableLightning extends Entity
{
    private int xTile = -1;
    private int yTile = -1;
    private int zTile = -1;
    ...
    public EntityThrowableLightning(World worldIn)
    {
        super(worldIn);
    ...
    public EntityThrowableLightning(World worldIn, double x, double y, double z, double accelX, double accelY, double accelZ)
    {
    	this(worldIn);
        this.setLocationAndAngles(x, y, z, this.rotationYaw, this.rotationPitch);
        this.setPosition(x, y, z);
        double d6 = (double)MathHelper.sqrt_double(accelX * accelX + accelY * accelY + accelZ * accelZ);
        this.accelerationX = accelX / d6 * 0.3D;
        this.accelerationY = accelY / d6 * 0.3D;
        this.accelerationZ = accelZ / d6 * 0.3D;
    }

    public EntityThrowableLightning(World worldIn, EntityPlayer shooter, double accelX, double accelY, double accelZ)
    {
        this(worldIn);
        ...
        this.accelerationX = accelX / d3 * 0.3D;
        this.accelerationY = accelY / d3 * 0.3D;
        this.accelerationZ = accelZ / d3 * 0.3D;
    }
    ...
    public void onUpdate()
    {
        ...
        {
    		this.worldObj.addWeatherEffect(new EntityLightningBolt(this.worldObj, this.getPosition().getX(), this.getPosition().getY(), this.getPosition().getZ()));
            this.setDead();
        }
    ...
    protected void onImpact(RayTraceResult movingObject)
    {
        if (!this.worldObj.isRemote)
        {
        	this.worldObj.addWeatherEffect(new EntityLightningBolt(this.worldObj, this.posX - 0.5D, this.posY, this.posZ - 0.5D));
            this.setDead();
        }
    }
