...
public class EntityWaterCurrent extends Entity
{
    private int xTile = -1;
    private int yTile = -1;
    private int zTile = -1;
    ...
    public EntityWaterCurrent(World worldIn)
    {
        super(worldIn);
    ...
    public EntityWaterCurrent(World worldIn, double x, double y, double z, double accelX, double accelY, double accelZ)
    {
    	this(worldIn);
        this.setSize(2.0F, 2.0F);
    ...
    public EntityWaterCurrent(World worldIn, EntityPlayer shooter, double accelX, double accelY, double accelZ)
    {
        this(worldIn);
    ...
    public void onUpdate()
    {
    	...
    	if (this.shootingEntity == null || (this.shootingEntity != null && this.getDistanceSqToEntity(this.shootingEntity) > 14400D))
        {
    	        ...
    	        if ((entity1 != null) && ((entity1 instanceof EntityLiving))) 
    	        {
    	        	if (this.shootingEntity != null)
    	        		if (entity1 instanceof EntityEnderman)
    	        			entity1.attackEntityFrom(DamageSource.causePlayerDamage((EntityPlayer)this.shootingEntity), 6F);
    	        		else
    	    ...
    	    if (this.shootingEntity != null)
            this.worldObj.newExplosion(this.shootingEntity, this.posX, this.posY, this.posZ, 2.0F, false, false);
            this.setDead();
        }
    ...
    protected void onImpact(RayTraceResult movingObject)
    {
        if (!this.worldObj.isRemote)
        {
            if (movingObject.entityHit != null)
            {
                if (this.shootingEntity != null)
                {
                	if (movingObject.entityHit instanceof EntityEnderman || movingObject.entityHit instanceof EntityDragon || movingObject.entityHit instanceof EntityDragonPart)
                	{
                		movingObject.entityHit.attackEntityFrom(DamageSource.causePlayerDamage((EntityPlayer)this.shootingEntity), 50.0F);
