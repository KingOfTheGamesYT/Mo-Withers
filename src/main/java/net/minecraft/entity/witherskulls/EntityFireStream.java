...
public class EntityFireStream extends Entity
{
    private int xTile = -1;
    private int yTile = -1;
    private int zTile = -1;
    ...
    public EntityFireStream(World worldIn, double x, double y, double z, double accelX, double accelY, double accelZ)
    {
    	this(worldIn);
        this.setSize(2.0F, 2.0F);
    ...
    public EntityFireStream(World worldIn, EntityPlayer shooter, double accelX, double accelY, double accelZ)
    {
        this(worldIn);
    ...
    public void onUpdate()
    {
    	if (this.shootingEntity == null || (this.shootingEntity != null && this.getDistanceSqToEntity(this.shootingEntity) > 14400D))
        {
    		if (this.shootingEntity != null)
                this.worldObj.newExplosion(this.shootingEntity, this.posX, this.posY + 2D, this.posZ, 2.0F, true, false);
                this.setDead();
        }
    	
    	if (!this.worldObj.isRemote)
    	{
    		this.worldObj.newExplosion(this.shootingEntity, this.posX, this.posY + 2D, this.posZ, 1.0F, true, true);
    	}
        ...
        else
        {
            ...
            Vec3d vec3 = new Vec3d(this.posX, this.posY + 1D, this.posZ);
            Vec3d vec31 = new Vec3d(this.posX + this.motionX, this.posY + 1D + this.motionY, this.posZ + this.motionZ);
        ...
        if (!this.worldObj.isRemote)
        {
    	    ...
    	    if ((list111 != null) && (!list111.isEmpty())) {
    	      for (int i111 = 0; i111 < list111.size(); i111++)
    	      {
    	        ...
    	        if ((entity1 != null) && !entity1.isImmuneToFire() && ((entity1 instanceof EntityLiving))) 
    	        {
    	        	if (this.shootingEntity != null)
    	    ...
    	    if (this.shootingEntity != null)
            this.worldObj.newExplosion(this.shootingEntity, this.posX, this.posY, this.posZ, 2.0F, true, false);
            this.setDead();
        }
    }
