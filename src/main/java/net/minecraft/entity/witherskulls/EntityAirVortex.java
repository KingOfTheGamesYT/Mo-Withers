...
public class EntityAirVortex extends Entity
{
    private int xTile = -1;
    private int yTile = -1;
    private int zTile = -1;
    ...
    public EntityAirVortex(World worldIn)
    {
        super(worldIn);
    ...
    public EntityAirVortex(World worldIn, double x, double y, double z, double accelX, double accelY, double accelZ)
    {
    	this(worldIn);
        this.setSize(2.0F, 2.0F);
    ...
    public EntityAirVortex(World worldIn, EntityPlayer shooter, double accelX, double accelY, double accelZ)
    {
        this(worldIn);
    ...
    public void onUpdate()
    {
        ...
        else
        {
            ...
            List list111 = this.worldObj.getEntitiesWithinAABBExcludingEntity(this, getEntityBoundingBox().expand(12.0D, 64.0D, 12.0D));
    	    if ((list111 != null) && (!list111.isEmpty())) {
    	      for (int i111 = 0; i111 < list111.size(); i111++)
    	      {
    	        ...
    	        if ((entity != null) && ((entity instanceof EntityLiving))) 
    	        {
	                ...
	                if (entity.getDistanceSqToCenter(new BlockPos(this.posX, this.posY + 6D, this.posZ)) <= 45D)
	                {
            ...
            {
            	List list1111 = this.worldObj.getEntitiesWithinAABBExcludingEntity(this, getEntityBoundingBox().expand(12.0D, 32.0D, 12.0D));
        	    ...
        	    if (this.shootingEntity != null)
                this.worldObj.newExplosion(this.shootingEntity, this.posX, this.posY + 2D, this.posZ, 4.0F, false, false);
                this.setDead();
            }
            ...
            this.worldObj.spawnParticle(EnumParticleTypes.EXPLOSION_LARGE, this.posX, this.posY, this.posZ, 0.0D, 0.0D, 0.0D, new int[0]);
            this.setPosition(this.posX, this.posY, this.posZ);
            if (this.shootingEntity != null)
            	this.worldObj.newExplosion(this.shootingEntity, this.posX, this.posY + 2D, this.posZ, 1.0F, false, false);
            else
            	this.worldObj.newExplosion(this, this.posX, this.posY + 2D, this.posZ, 1.0F, false, false);
        }
    }
